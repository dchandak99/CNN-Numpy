...
class ConvolutionLayer:
    ...
    def forwardpass(self, X):
        ...
        if self.activation == 'relu':
            ...
            #self.data = relu_of_X(outpu.reshape([n, -1])).reshape(outpu.shape)
            return self.data
        else:
            print("ERROR: Incorrect activation specified: " + self.activation)
            exit()
    ...
    def backwardpass(self, lr, activation_prev, delta):
        ...
        for i in range(self.out_row):
            for j in range(self.out_col):
                ...
                new_delta[:, :, i_start:i_end, j_start:j_end] = new_delta[:, :, i_start:i_end, j_start:j_end] + conv_back
    ...
    def backwardpass(self, alpha, activation_prev, delta):
        ...
        for i in range(self.out_row):
            for j in range(self.out_col):
                ...
                outpu[:, :, i_start:i_end, j_start:j_end] += \
                    delta[:, :, i:i + 1, j:j + 1] * self._cache[(i, j)]
...
class FlattenLayer:
    ...
    def forwardpass(self, X):
        ...
        self.in_batch, self.ro, self.co, self.k = X.shape
        return X.reshape((self.in_batch, self.ro * self.co * self.k))
        
        # print(X.shape)
        pass
    def backwardpass(self, lr, activation_prev, delta):
        
        return delta.reshape((self.in_batch, self.ro, self.co, self.k))
